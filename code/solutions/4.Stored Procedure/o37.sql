delimiter //
CREATE PROCEDURE o37 (IN ut INT,IN vend VARCHAR(16),OUT esito BOOLEAN) 
BEGIN 
	DECLARE X INT DEFAULT 5;
	DECLARE W INT DEFAULT 4;
	DECLARE Q INT DEFAULT 5;
	DECLARE c CURSOR FOR (SELECT Cod_Mes
						  FROM Invia_C_Ut AS i,Messaggi_Chat AS m,Riceve_C_Ve AS r
	...
	IF (X=5) THEN
		REPEAT
			FETCH c INTO W;
			IF (X=5) THEN
				UPDATE Invia_C_Ut SET Cancellato=TRUE WHERE Cod_Mes=W;
			END IF;
	...
	IF (X=5) THEN
		REPEAT
			FETCH d INTO Q;
			IF (X=5) THEN
				UPDATE Riceve_C_Ut SET Cancellato=TRUE WHERE Cod_Mes=Q;
			END IF;
		UNTIL (X=0) END REPEAT;
	END IF;
	CLOSE d;
